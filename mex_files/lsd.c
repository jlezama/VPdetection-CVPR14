...
/*----------------------------------------------------------------------------*/
/** LSD Matlab Interface.
    Author: jlezama@gmail.com
                 ...
                 int nrhs, const mxArray *prhs[])
{
  ...
  /* 
     ...
     output: 7xN matrix with info for N line segments
  */

  /* type checking */
  if (mxGetNumberOfDimensions(prhs[0])!=2){
    mexErrMsgIdAndTxt("MyToolbox:arrayProduct:notRowVector",
		      "Input matrix must be 2D (grayscale image).");
  }
  ...
  double * img = mxGetPr(prhs[0]); /* img */
  int X = mxGetN(prhs[0]); /* number of columns */
  int Y = mxGetM(prhs[0]); /* number of rows */
  
  int n_out; 

  /* execute LSD */
  double * lines = lsd(&n_out, img, Y, X); /* ?? does not correspond to LSD doc*/



  /* output variables definitions */
  int lw = 7; /* x1,y1,x2,y2,width,p,-log10(NFA) */
