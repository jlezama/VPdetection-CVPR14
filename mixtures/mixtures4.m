function [bestk,bestpp,bestmu,bestcov,dl,countf, bestpairs] = mixtures4(y,kmin,kmax,regularize,th,covoption, npoints)
%
%
% Usage syntax:
...
while(k_cont)  % the outer loop will take us down from kmax to kmin components
    ...
    while(cont)    % this inner loop is the component-wise EM algorithm with the
        ...
        while comp <= k
            ...
            %             end
            %
            indic = my_repmat(estpp',[1 size(semi_indic,2)]).*semi_indic;
            
            % normindic = indic./(realmin+my_kron(ones(k,1),sum(indic,1)));
            % % slow, original
            normindic = indic./(realmin+my_repmat2(sum(indic,1),[k 1])); % fast, mine
            
            % now we perform the standard M-step for mean and covariance
            normalize = 1/sum(normindic(comp,:));
            
            %aux = my_kron(normindic(comp,:),ones(dimens,1)).*y; % slow,
            %original
            aux = my_repmat2(normindic(comp,:),[dimens 1]).*y; % fast, mine
            ...
            % parameter estimates
            if estpp(comp)==0 || isnan(estpp(comp))
                killed = 1;
            ...
            %                 for kk = 1:k
            %                    semi_indic(kk,:) = my_multinorm(y,estmu(:,kk),estcov(:,:,kk));
            %                 end
            %              end
            ...
            if killed==0
                ...
                semi_indic(comp,:) = my_multinorm(y,estmu(:,comp),estcov(:,:,comp), npoints);
...
if PLOT
    ...
    if dimens == 1
        ...
        text(placex(1)+1,placey(2)-1,sprintf('k=%d',length(bestpp)),...
            'FontName','Times','FontSize',22);
        bestpairs = zeros(length(bestpp),4);
        for comp=1:length(bestpp)
            bestpair = elipsnorm(bestmu([axis1,axis2],comp),bestcov([axis1,axis2],[axis1,axis2],comp),2);
...
%% get elipses endpoints
bestpairs = zeros(length(bestpp),4);

for comp=1:length(bestpp)
    bestpair = get_ellipse_endpoints(bestmu([axis1,axis2],comp),bestcov([axis1,axis2],[axis1,axis2],comp),2);
    bestpairs(comp,:) = bestpair;
end
